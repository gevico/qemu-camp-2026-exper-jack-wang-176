...
void qdev_init_gpio_in_named_with_opaque(DeviceState *dev,
  ...
  gpio_list->in =
      qemu_extend_irqs(gpio_list->in, gpio_list->num_in, handler, opaque, n);
...
void qdev_init_gpio_out_named(DeviceState *dev, qemu_irq *pins,
  ...
  for (i = 0; i < n; ++i) {
    ...
    object_property_add_link(
        OBJECT(dev), propname, TYPE_IRQ, (Object **)&pins[i],
...
void qdev_connect_gpio_out_named(DeviceState *dev, const char *name, int n,
                                 qemu_irq input_pin) {
  char *propname =
      g_strdup_printf("%s[%d]", name ? name : "unnamed-gpio-out", n);
...
qemu_irq qdev_get_gpio_out_connector(const DeviceState *dev, const char *name,
                                     int n) {
  g_autofree char *propname =
      g_strdup_printf("%s[%d]", name ? name : "unnamed-gpio-out", n);

  qemu_irq ret =
      (qemu_irq)object_property_get_link(OBJECT(dev), propname, NULL);
...
void qdev_pass_gpios(DeviceState *dev, DeviceState *container,
  ...
  for (i = 0; i < ngl->num_in; i++) {
    ...
    object_property_add_alias(OBJECT(container), propname, OBJECT(dev),
                              propname);
    g_free(propname);
  }
