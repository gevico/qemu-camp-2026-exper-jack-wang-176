...
static PFlashCFI01 *virt_flash_create1(RISCVG233State *s, const char *name,
...
static void virt_flash_map(RISCVG233State *s, MemoryRegion *sysmem) {
  hwaddr flashsize = s->memmap[VIRT_FLASH].size / 2;
  hwaddr flashbase = s->memmap[VIRT_FLASH].base;

  virt_flash_map1(s->flash[0], flashbase, flashsize, sysmem);
  virt_flash_map1(s->flash[1], flashbase + flashsize, flashsize, sysmem);
}
...
static void create_pcie_irq_map(RISCVG233State *s, void *fdt, char *nodename,
  ...
  uint32_t full_irq_map[PCI_NUM_PINS * PCI_NUM_PINS * FDT_MAX_INT_MAP_WIDTH] =
      {};
  ...
  qemu_fdt_setprop(fdt, nodename, "interrupt-map", full_irq_map,
                   PCI_NUM_PINS * PCI_NUM_PINS * irq_map_stride *
                       sizeof(uint32_t));
...
static void create_fdt_socket_cpus(RISCVG233State *s, int socket,
  ...
  for (cpu = s->soc[socket].num_harts - 1; cpu >= 0; cpu--) {
    ...
    cpu_name =
        g_strdup_printf("/cpus/cpu@%d", s->soc[socket].hartid_base + cpu);
    qemu_fdt_add_subnode(ms->fdt, cpu_name);

    if (satp_mode_max != -1) {
      sv_name =
          g_strdup_printf("riscv,%s", satp_mode_str(satp_mode_max, is_32_bit));
    ...
    qemu_fdt_setprop_cell(ms->fdt, intc_name, "phandle", intc_phandles[cpu]);
...
static void create_fdt_socket_clint(RISCVG233State *s, int socket,
                                    uint32_t *intc_phandles) {
  int cpu;
  ...
  hwaddr clint_addr;
  MachineState *ms = MACHINE(s);
  static const char *const clint_compat[2] = {"sifive,clint0", "riscv,clint0"};
  ...
  clint_addr = s->memmap[VIRT_CLINT].base + s->memmap[VIRT_CLINT].size * socket;
  clint_name = g_strdup_printf("/soc/clint@%" HWADDR_PRIx, clint_addr);
  qemu_fdt_add_subnode(ms->fdt, clint_name);
  qemu_fdt_setprop_string_array(ms->fdt, clint_name, "compatible",
                                (char **)&clint_compat,
                                ARRAY_SIZE(clint_compat));
  qemu_fdt_setprop_sized_cells(ms->fdt, clint_name, "reg", 2, clint_addr, 2,
...
static void create_fdt_socket_aclint(RISCVG233State *s, int socket,
                                     uint32_t *intc_phandles) {
  int cpu;
  char *name;
  ...
  if (s->aia_type != G233_AIA_TYPE_APLIC_IMSIC) {
    addr = s->memmap[VIRT_CLINT].base + (s->memmap[VIRT_CLINT].size * socket);
    name = g_strdup_printf("/soc/mswi@%lx", addr);

    qemu_fdt_add_subnode(ms->fdt, name);
    qemu_fdt_setprop_string(ms->fdt, name, "compatible", "riscv,aclint-mswi");
    qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2, addr, 2,
                                 RISCV_ACLINT_SWI_SIZE);
    qemu_fdt_setprop(ms->fdt, name, "interrupts-extended", aclint_mswi_cells,
                     aclint_cells_size);
  ...
  qemu_fdt_add_subnode(ms->fdt, name);
  qemu_fdt_setprop_string(ms->fdt, name, "compatible", "riscv,aclint-mtimer");
  qemu_fdt_setprop_sized_cells(
      ms->fdt, name, "reg", 2, addr + RISCV_ACLINT_DEFAULT_MTIME, 2,
      size - RISCV_ACLINT_DEFAULT_MTIME, 2,
      addr + RISCV_ACLINT_DEFAULT_MTIMECMP, 2, RISCV_ACLINT_DEFAULT_MTIME);
  qemu_fdt_setprop(ms->fdt, name, "interrupts-extended", aclint_mtimer_cells,
                   aclint_cells_size);
  riscv_socket_fdt_write_id(ms, name, socket);
  g_free(name);
  ...
  if (s->aia_type != G233_AIA_TYPE_APLIC_IMSIC) {
    ...
    qemu_fdt_add_subnode(ms->fdt, name);
    qemu_fdt_setprop_string(ms->fdt, name, "compatible", "riscv,aclint-sswi");
    qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2, addr, 2,
                                 s->memmap[VIRT_ACLINT_SSWI].size);
    qemu_fdt_setprop(ms->fdt, name, "interrupts-extended", aclint_sswi_cells,
                     aclint_cells_size);
    qemu_fdt_setprop(ms->fdt, name, "interrupt-controller", NULL, 0);
    qemu_fdt_setprop_cell(ms->fdt, name, "#interrupt-cells", 0);
    riscv_socket_fdt_write_id(ms, name, socket);
...
static void create_fdt_socket_plic(RISCVG233State *s, int socket,
  ...
  MachineState *ms = MACHINE(s);
  static const char *const plic_compat[2] = {"sifive,plic-1.0.0",
                                             "riscv,plic0"};

  plic_phandles[socket] = (*phandle)++;
  plic_addr = s->memmap[VIRT_PLIC].base + (s->memmap[VIRT_PLIC].size * socket);
  plic_name = g_strdup_printf("/soc/plic@%lx", plic_addr);
  qemu_fdt_add_subnode(ms->fdt, plic_name);
  qemu_fdt_setprop_cell(ms->fdt, plic_name, "#interrupt-cells",
                        FDT_PLIC_INT_CELLS);
  qemu_fdt_setprop_cell(ms->fdt, plic_name, "#address-cells",
                        FDT_PLIC_ADDR_CELLS);
  ...
  qemu_fdt_setprop_sized_cells(ms->fdt, plic_name, "reg", 2, plic_addr, 2,
                               s->memmap[VIRT_PLIC].size);
  qemu_fdt_setprop_cell(ms->fdt, plic_name, "riscv,ndev",
                        VIRT_IRQCHIP_NUM_SOURCES - 1);
  riscv_socket_fdt_write_id(ms, plic_name, socket);
  qemu_fdt_setprop_cell(ms->fdt, plic_name, "phandle", plic_phandles[socket]);

  if (!socket) {
    platform_bus_add_all_fdt_nodes(
        ms->fdt, plic_name, s->memmap[VIRT_PLATFORM_BUS].base,
        s->memmap[VIRT_PLATFORM_BUS].size, VIRT_PLATFORM_BUS_IRQ);
  }
}
...
static void create_fdt_one_imsic(RISCVG233State *s, hwaddr base_addr,
  ...
  static const char *const imsic_compat[2] = {"qemu,imsics", "riscv,imsics"};
  ...
  for (socket = 0; socket < socket_count; socket++) {
    ...
    imsic_size = IMSIC_HART_SIZE(imsic_guest_bits) * s->soc[socket].num_harts;
  ...
  qemu_fdt_setprop(ms->fdt, imsic_name, "interrupts-extended", imsic_cells,
...
static void create_fdt_imsic(RISCVG233State *s, uint32_t *phandle,
                             uint32_t *intc_phandles, uint32_t *msi_m_phandle,
...
static void create_fdt_one_aplic(RISCVG233State *s, int socket,
                                 ...
                                 uint32_t msi_phandle, uint32_t *intc_phandles,
                                 uint32_t aplic_phandle,
                                 uint32_t aplic_child_phandle, bool m_mode,
                                 int num_harts) {
  int cpu;
  g_autofree char *aplic_name = fdt_get_aplic_nodename(aplic_addr);
  g_autofree uint32_t *aplic_cells = g_new0(uint32_t, num_harts * 2);
  MachineState *ms = MACHINE(s);
  static const char *const aplic_compat[2] = {"qemu,aplic", "riscv,aplic"};
  ...
  if (s->aia_type == G233_AIA_TYPE_APLIC) {
    qemu_fdt_setprop(ms->fdt, aplic_name, "interrupts-extended", aplic_cells,
                     num_harts * sizeof(uint32_t) * 2);
  } else {
    qemu_fdt_setprop_cell(ms->fdt, aplic_name, "msi-parent", msi_phandle);
  }

  qemu_fdt_setprop_sized_cells(ms->fdt, aplic_name, "reg", 2, aplic_addr, 2,
                               aplic_size);
...
static void create_fdt_socket_aplic(RISCVG233State *s, int socket,
                                    uint32_t msi_m_phandle,
                                    uint32_t msi_s_phandle, uint32_t *phandle,
                                    uint32_t *intc_phandles,
                                    uint32_t *aplic_phandles, int num_harts) {
  ...
  if (!kvm_enabled()) {
    /* M-level APLIC node */
    aplic_addr =
        s->memmap[VIRT_APLIC_M].base + (s->memmap[VIRT_APLIC_M].size * socket);
    create_fdt_one_aplic(s, socket, aplic_addr, s->memmap[VIRT_APLIC_M].size,
                         msi_m_phandle, intc_phandles, aplic_m_phandle,
                         aplic_s_phandle, true, num_harts);
  }

  /* S-level APLIC node */
  aplic_addr =
      s->memmap[VIRT_APLIC_S].base + (s->memmap[VIRT_APLIC_S].size * socket);
  create_fdt_one_aplic(s, socket, aplic_addr, s->memmap[VIRT_APLIC_S].size,
                       msi_s_phandle, intc_phandles, aplic_s_phandle, 0, false,
                       num_harts);

  if (!socket) {
    g_autofree char *aplic_name = fdt_get_aplic_nodename(aplic_addr);
    platform_bus_add_all_fdt_nodes(
        ms->fdt, aplic_name, s->memmap[VIRT_PLATFORM_BUS].base,
        s->memmap[VIRT_PLATFORM_BUS].size, VIRT_PLATFORM_BUS_IRQ);
  }
...
static void create_fdt_sockets(RISCVG233State *s, uint32_t *phandle,
  ...
  qemu_fdt_setprop_cell(
      ms->fdt, "/cpus", "timebase-frequency",
  ...
  for (socket = (socket_count - 1); socket >= 0; socket--) {
    ...
    if (virt_aclint_allowed() && s->have_aclint) {
      create_fdt_socket_aclint(s, socket, &intc_phandles[phandle_pos]);
    } else if (tcg_enabled()) {
      create_fdt_socket_clint(s, socket, &intc_phandles[phandle_pos]);
    }
  }

  if (s->aia_type == G233_AIA_TYPE_APLIC_IMSIC) {
    create_fdt_imsic(s, phandle, intc_phandles, &msi_m_phandle, &msi_s_phandle);
  ...
  if (!virt_use_emulated_aplic(s->aia_type)) {
    create_fdt_socket_aplic(s, 0, msi_m_phandle, msi_s_phandle, phandle,
    ...
    *irq_pcie_phandle = xplic_phandles[0];
    ...
    for (socket = (socket_count - 1); socket >= 0; socket--) {
      ...
      if (s->aia_type == G233_AIA_TYPE_NONE) {
        create_fdt_socket_plic(s, socket, phandle, &intc_phandles[phandle_pos],
                               xplic_phandles);
      } else {
        create_fdt_socket_aplic(s, socket, msi_m_phandle, msi_s_phandle,
                                phandle, &intc_phandles[phandle_pos],
                                xplic_phandles, s->soc[socket].num_harts);
      }
    }
...
static void create_fdt_virtio(RISCVG233State *s, uint32_t irq_virtio_phandle) {
  ...
  for (i = 0; i < G233_VIRTIO_COUNT; i++) {
    ...
    qemu_fdt_setprop_cell(ms->fdt, name, "interrupt-parent",
                          irq_virtio_phandle);
    if (s->aia_type == G233_AIA_TYPE_NONE) {
      qemu_fdt_setprop_cell(ms->fdt, name, "interrupts", G233_VIRTIO_IRQ + i);
    } else {
      qemu_fdt_setprop_cells(ms->fdt, name, "interrupts", G233_VIRTIO_IRQ + i,
                             0x4);
    }
  }
}
...
static void create_fdt_pcie(RISCVG233State *s, uint32_t irq_pcie_phandle,
                            uint32_t msi_pcie_phandle,
  ...
  name =
      g_strdup_printf("/soc/pci@%" HWADDR_PRIx, s->memmap[VIRT_PCIE_ECAM].base);
  ...
  qemu_fdt_setprop_string(ms->fdt, name, "compatible", "pci-host-ecam-generic");
  ...
  qemu_fdt_setprop_cells(ms->fdt, name, "bus-range", 0,
                         s->memmap[VIRT_PCIE_ECAM].size / PCIE_MMCFG_SIZE_MIN -
                             1);
  ...
  qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2,
                               s->memmap[VIRT_PCIE_ECAM].base, 2,
                               s->memmap[VIRT_PCIE_ECAM].size);
  qemu_fdt_setprop_sized_cells(
      ms->fdt, name, "ranges", 1, FDT_PCI_RANGE_IOPORT, 2, 0, 2,
      s->memmap[VIRT_PCIE_PIO].base, 2, s->memmap[VIRT_PCIE_PIO].size, 1,
      FDT_PCI_RANGE_MMIO, 2, s->memmap[VIRT_PCIE_MMIO].base, 2,
      s->memmap[VIRT_PCIE_MMIO].base, 2, s->memmap[VIRT_PCIE_MMIO].size, 1,
      FDT_PCI_RANGE_MMIO_64BIT, 2, virt_high_pcie_memmap.base, 2,
      virt_high_pcie_memmap.base, 2, virt_high_pcie_memmap.size);

  if (g233_is_iommu_sys_enabled(s)) {
    qemu_fdt_setprop_cells(ms->fdt, name, "iommu-map", 0, iommu_sys_phandle, 0,
                           0, 0, iommu_sys_phandle, 0, 0xffff);
  }
...
static void create_fdt_reset(RISCVG233State *s, uint32_t *phandle) {
  ...
  name = g_strdup_printf("/soc/test@%" HWADDR_PRIx, s->memmap[VIRT_TEST].base);
  qemu_fdt_add_subnode(ms->fdt, name);
  {
    static const char *const compat[3] = {"sifive,test1", "sifive,test0",
                                          "syscon"};
    qemu_fdt_setprop_string_array(ms->fdt, name, "compatible", (char **)&compat,
                                  ARRAY_SIZE(compat));
  }
  qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2,
                               s->memmap[VIRT_TEST].base, 2,
...
static void create_fdt_uart(RISCVG233State *s, uint32_t irq_mmio_phandle) {
  g_autofree char *name = NULL;
  MachineState *ms = MACHINE(s);

  name =
      g_strdup_printf("/soc/serial@%" HWADDR_PRIx, s->memmap[VIRT_UART0].base);
  qemu_fdt_add_subnode(ms->fdt, name);
  // qemu_fdt_setprop_string(ms->fdt, name, "compatible", "ns16550a");
  qemu_fdt_setprop_string(ms->fdt, name, "compatible", "pl011");
  qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2,
                               s->memmap[VIRT_UART0].base, 2,
                               s->memmap[VIRT_UART0].size);
...
static void create_fdt_rtc(RISCVG233State *s, uint32_t irq_mmio_phandle) {
  g_autofree char *name = NULL;
  MachineState *ms = MACHINE(s);

  name = g_strdup_printf("/soc/rtc@%" HWADDR_PRIx, s->memmap[VIRT_RTC].base);
  qemu_fdt_add_subnode(ms->fdt, name);
  qemu_fdt_setprop_string(ms->fdt, name, "compatible", "google,goldfish-rtc");
  qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2,
                               s->memmap[VIRT_RTC].base, 2,
                               s->memmap[VIRT_RTC].size);
  qemu_fdt_setprop_cell(ms->fdt, name, "interrupt-parent", irq_mmio_phandle);
  if (s->aia_type == G233_AIA_TYPE_NONE) {
...
static void create_fdt_flash(RISCVG233State *s) {
  ...
  qemu_fdt_setprop_sized_cells(ms->fdt, name, "reg", 2, flashbase, 2, flashsize,
...
static void create_fdt_virtio_iommu(RISCVG233State *s, uint16_t bdf) {
  ...
  pci_node =
      g_strdup_printf("/soc/pci@%" HWADDR_PRIx, s->memmap[VIRT_PCIE_ECAM].base);
  iommu_node = g_strdup_printf("%s/virtio_iommu@%x,%x", pci_node, PCI_SLOT(bdf),
                               PCI_FUNC(bdf));
  ...
  qemu_fdt_setprop_sized_cells(fdt, iommu_node, "reg", 1, bdf << 8, 1, 0, 1, 0,
                               1, 0, 1, 0);
  qemu_fdt_setprop_cell(fdt, iommu_node, "#iommu-cells", 1);
  qemu_fdt_setprop_cell(fdt, iommu_node, "phandle", iommu_phandle);

  qemu_fdt_setprop_cells(fdt, pci_node, "iommu-map", 0, iommu_phandle, 0, bdf,
...
static void create_fdt_iommu_sys(RISCVG233State *s, uint32_t irq_chip,
  ...
  qemu_fdt_setprop_cells(
      fdt, iommu_node, "interrupts", iommu_irq_map[0], FDT_IRQ_TYPE_EDGE_LOW,
      iommu_irq_map[1], FDT_IRQ_TYPE_EDGE_LOW, iommu_irq_map[2],
      FDT_IRQ_TYPE_EDGE_LOW, iommu_irq_map[3], FDT_IRQ_TYPE_EDGE_LOW);
...
static void create_fdt_iommu(RISCVG233State *s, uint16_t bdf) {
  ...
  qemu_fdt_setprop_cell(fdt, iommu_node, "#iommu-cells", 1);
  qemu_fdt_setprop_cell(fdt, iommu_node, "phandle", iommu_phandle);
  qemu_fdt_setprop_cells(fdt, iommu_node, "reg", bdf << 8, 0, 0, 0, 0);
  qemu_fdt_setprop_cells(fdt, pci_node, "iommu-map", 0, iommu_phandle, 0, bdf,
                         bdf + 1, iommu_phandle, bdf + 1, 0xffff - bdf);
  s->pci_iommu_bdf = bdf;
}
...
static void finalize_fdt(RISCVG233State *s) {
  ...
  create_fdt_sockets(s, &phandle, &irq_mmio_phandle, &irq_pcie_phandle,
  ...
  create_fdt_pcie(s, irq_pcie_phandle, msi_pcie_phandle, iommu_sys_phandle);
...
gpex_pcie_init(MemoryRegion *sys_mem, DeviceState *irqchip, RISCVG233State *s) {
  DeviceState *dev;
  ...
  object_property_set_uint(OBJECT(dev), PCI_HOST_BELOW_4G_MMIO_BASE, mmio_base,
                           NULL);
  object_property_set_int(OBJECT(dev), PCI_HOST_BELOW_4G_MMIO_SIZE, mmio_size,
                          NULL);
  ...
  object_property_set_int(OBJECT(dev), PCI_HOST_ABOVE_4G_MMIO_SIZE,
                          high_mmio_size, NULL);
  object_property_set_uint(OBJECT(dev), PCI_HOST_PIO_BASE, pio_base, NULL);
  object_property_set_int(OBJECT(dev), PCI_HOST_PIO_SIZE, pio_size, NULL);
...
static FWCfgState *create_fw_cfg(const MachineState *ms, hwaddr base) {
  ...
  fw_cfg =
      fw_cfg_init_mem_dma(base + 8, base, 8, base + 16, &address_space_memory);
...
static DeviceState *virt_create_plic(const MemMapEntry *memmap, int socket,
  ...
  return sifive_plic_create(
      ...
      plic_hart_config, hart_count, base_hartid, VIRT_IRQCHIP_NUM_SOURCES,
      ((1U << VIRT_IRQCHIP_NUM_PRIO_BITS) - 1), VIRT_PLIC_PRIORITY_BASE,
      VIRT_PLIC_PENDING_BASE, VIRT_PLIC_ENABLE_BASE, VIRT_PLIC_ENABLE_STRIDE,
      VIRT_PLIC_CONTEXT_BASE, VIRT_PLIC_CONTEXT_STRIDE, memmap[VIRT_PLIC].size);
}
...
static DeviceState *virt_create_aia(RISCVG233AIAType aia_type, int aia_guests,
  ...
  if (msimode) {
    if (!kvm_enabled()) {
      ...
      for (i = 0; i < hart_count; i++) {
        riscv_imsic_create(addr + i * IMSIC_HART_SIZE(0), base_hartid + i, true,
                           1, VIRT_IRQCHIP_NUM_MSIS);
      }
    }
  ...
  if (!kvm_enabled()) {
    ...
    aplic_m = riscv_aplic_create(
        ...
        memmap[VIRT_APLIC_M].size, (msimode) ? 0 : base_hartid,
        (msimode) ? 0 : hart_count, VIRT_IRQCHIP_NUM_SOURCES,
        VIRT_IRQCHIP_NUM_PRIO_BITS, msimode, true, NULL);
  }

  /* Per-socket S-level APLIC */
  aplic_s = riscv_aplic_create(
      memmap[VIRT_APLIC_S].base + socket * memmap[VIRT_APLIC_S].size,
      memmap[VIRT_APLIC_S].size, (msimode) ? 0 : base_hartid,
      (msimode) ? 0 : hart_count, VIRT_IRQCHIP_NUM_SOURCES,
      VIRT_IRQCHIP_NUM_PRIO_BITS, msimode, false, aplic_m);
...
static void create_platform_bus(RISCVG233State *s, DeviceState *irqchip) {
  ...
  memory_region_add_subregion(sysmem, s->memmap[VIRT_PLATFORM_BUS].base,
...
static void virt_build_smbios(RISCVG233State *s) {
  ...
  smbios_get_tables(ms, SMBIOS_ENTRY_POINT_TYPE_64, &mem_array, 1,
                    &smbios_tables, &smbios_tables_len, &smbios_anchor,
                    &smbios_anchor_len, &error_fatal);

  if (smbios_anchor) {
    fw_cfg_add_file(s->fw_cfg, "etc/smbios/smbios-tables", smbios_tables,
                    smbios_tables_len);
    fw_cfg_add_file(s->fw_cfg, "etc/smbios/smbios-anchor", smbios_anchor,
                    smbios_anchor_len);
  }
}

static void virt_machine_done(Notifier *notifier, void *data) {
  RISCVG233State *s = container_of(notifier, RISCVG233State, machine_done);
  ...
  fdt_load_addr =
      riscv_compute_fdt_addr(s->memmap[VIRT_DRAM].base,
                             s->memmap[VIRT_DRAM].size, machine, &boot_info);
  riscv_load_fdt(fdt_load_addr, machine->fdt);

  /* load the reset vector */
  riscv_setup_rom_reset_vec(
      machine, &s->soc[0], start_addr, s->memmap[VIRT_MROM].base,
      s->memmap[VIRT_MROM].size, kernel_entry, fdt_load_addr);
...
static void virt_machine_init(MachineState *machine) {
  ...
  for (i = 0; i < socket_count; i++) {
    ...
    if (virt_aclint_allowed() && s->have_aclint) {
      if (s->aia_type == G233_AIA_TYPE_APLIC_IMSIC) {
        ...
        riscv_aclint_mtimer_create(
            s->memmap[VIRT_CLINT].base + i * RISCV_ACLINT_DEFAULT_MTIMER_SIZE,
    ...
    if (s->aia_type == G233_AIA_TYPE_NONE) {
      ...
      s->irqchip[i] = virt_create_aia(s->aia_type, s->aia_guests, s->memmap, i,
  ...
  if (kvm_enabled() && g233_use_kvm_aia_aplic_imsic(s->aia_type)) {
    kvm_riscv_aia_create(machine, IMSIC_MMIO_GROUP_MIN_SHIFT,
                         ...
                         s->memmap[VIRT_IMSIC_S].base, s->aia_guests);
  }
  ...
  if (riscv_is_32bit(&s->soc[0])) {
    ...
    virt_high_pcie_memmap.base = s->memmap[VIRT_DRAM].base + machine->ram_size;
  ...
  for (i = 0; i < G233_VIRTIO_COUNT; i++) {
    sysbus_create_simple("virtio-mmio",
                         s->memmap[VIRT_VIRTIO].base +
                             i * s->memmap[VIRT_VIRTIO].size,
                         qdev_get_gpio_in(virtio_irqchip, G233_VIRTIO_IRQ + i));
  }
  ...
  pl011_create(s->memmap[VIRT_UART0].base,
               qdev_get_gpio_in(mmio_irqchip, UART0_IRQ), serial_hd(0));
  ...
  if (g233_is_iommu_sys_enabled(s)) {
    ...
    object_property_set_uint(OBJECT(iommu_sys), "addr",
                             s->memmap[VIRT_IOMMU_SYS].base, &error_fatal);
    object_property_set_uint(OBJECT(iommu_sys), "base-irq", IOMMU_SYS_IRQ,
                             &error_fatal);
    object_property_set_link(OBJECT(iommu_sys), "irqchip", OBJECT(mmio_irqchip),
                             &error_fatal);
  ...
  // 创建并连接另一个flash设备，w25x16
  // 作为上层模型驱动，rust后续需要暴露相应总线
  ...
  // 添加gpio控制实体
  DeviceState *gpio_dev = qdev_new("gpio");
  SysBusDevice *gpio_sbd = SYS_BUS_DEVICE(gpio_dev);
  // 挂载mmio内存空间和中断线
  ...
  qemu_irq rspi_flash_cs_irq =
      qdev_get_gpio_in_named(rspi_flash, SSI_GPIO_CS, 0);
...
static void virt_machine_class_init(ObjectClass *oc, const void *data) {
  ...
  object_class_property_add_str(oc, "aia", virt_get_aia, virt_set_aia);
  object_class_property_set_description(oc, "aia",
                                        "Set type of AIA interrupt "
                                        "controller. Valid values are "
                                        "none, aplic, and aplic-imsic.");

  object_class_property_add_str(oc, "aia-guests", virt_get_aia_guests,
                                virt_set_aia_guests);
  {
    g_autofree char *str =
  ...
  object_class_property_add(oc, "iommu-sys", "OnOffAuto", virt_get_iommu_sys,
                            virt_set_iommu_sys, NULL, NULL);
